...
{
#ifdef __EMSCRIPTEN__
  // mount the real filesystem under '/root'
  EM_ASM(
    FS.mkdir('/root');
...
#ifdef __EMSCRIPTEN__
    char *_org = (char*)calloc(1024, 1);
    char *_new = (char*)calloc(1024, 1);
    char *_out = (char*)calloc(1024, 1);

    lcFilNamOrg = (char*)EM_ASM_INT({
        // resolve path first
        let Path = require('path');
        let jspath = Path.join('/root', Path.resolve(UTF8ToString($0)));

        // copy to heap
        let len = lengthBytesUTF8(jspath) + 1;
        var strOnHeap = _malloc(len);
        stringToUTF8(jspath, strOnHeap, len);

        // return ptr
        return strOnHeap;
    }, lcFilNamOrg);

    lcFilNamNew = (char*)EM_ASM_INT({
        // resolve path first
        let Path = require('path');
        let jspath = Path.join('/root', Path.resolve(UTF8ToString($0)));
